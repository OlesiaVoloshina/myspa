...
export function CustomContent(props: BrProps) {
  ...
  const {buttonLabel, buttonLink, buttonCssClass} = props.component.getParameters();

  let buttonSummaryCss = "btn";
  if(buttonCssClass) {
  	buttonSummaryCss += " " + buttonCssClass;
  }
  ...
  return (
    <div className={props.page.isPreview() ? 'has-edit-button' : ''}>
      ...
      { geolocation && <p className="mb-3 small text-muted">Location: ({geolocation.latitude}, {geolocation.longitude})</p> }
      { buttonLabel && <a className={buttonSummaryCss} role="button" href={buttonLink}>{buttonLabel}</a>}
